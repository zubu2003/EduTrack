...
class StudentDashboardController extends GetxController {
  ...
  // Selected day in Class Schedule section
  RxString selectedScheduleDay = 'Sun'.obs;
  ...
  Future<void> fetchDashboardData() async {
    try {
      ...
      selectedScheduleDay.value = _todayDayName();
  ...
  /// Returns ALL routine entries for a given day (including Others)
  /// Used by Class Schedule section
  List<RoutineModel> getRoutineForDay(String day) {
    final list = allRoutines.where((r) => r.day == day).toList();
    list.sort((a, b) => _timeToMinutes(a.startTime)
        .compareTo(_timeToMinutes(b.startTime)));
    return list;
  }

  void selectScheduleDay(String day) {
  ...
  Future<void> openCourseDetails(RoutineModel routine) async {
    try {
      ...
      Get.to(() => StudentCourseDetailsScreen(course: course));
    } catch (e) {
      // Silent
    }
  }

  void openRoutine({String? day}) {
    Get.toNamed(
      AppRoutes.studentRoutine,
      arguments: {'initialDay': day ?? selectedScheduleDay.value},
    );
  }
